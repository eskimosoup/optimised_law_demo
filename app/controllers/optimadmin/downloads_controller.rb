module Optimadmin
  class DownloadsController < Optimadmin::ApplicationController
    ...
    edit_images_for Download, [[:image, { index: ['fill', 386, 150] }]]
  ...
  private
    ...
    def download_params
      params.require(:download).permit(:title, :image, :remote_image_url, :image_cache, :remove_image, :summary, :image, :remote_image_url, :image_cache, :remove_image, service_ids: [])
    end
  end
end
